...
func renderText(ctx *RenderContext, text string, array []any) error {
	...
	metricsOK := fontWidthMetricsAvailable(textState.Font)
	...
	enablePSSymbolGapClamp := true
	if enablePSSymbolGapClamp && psPreferText && psCtx != nil && candidateText != "" && !metricsOK {
		decoded, cids, _ := decodeTextStringWithCIDs(candidateText, toUnicodeMap, textState.Font)
		trimmed := strings.TrimSpace(decoded)
		runes := []rune(trimmed)
		if len(runes) > 0 && len(runes) <= 2 {
			symbolOnly := true
			hasUnicodeSymbol := false
			hasPunct := false
			for _, r := range runes {
				if unicode.IsMark(r) {
					symbolOnly = false
				...
				switch r {
				case '^', 0x02C6, 0x02C7, 0x02C9, 0x02D8, 0x02D9, 0x02DA, 0x02DB, 0x02DC, 0x00A8, 0x00B4, 0x0060:
					symbolOnly = false
					break
				}
				...
				mathish := unicode.IsSymbol(r) ||
					(r >= 0x2190 && r <= 0x21FF) ||
					(r >= 0x2200 && r <= 0x22FF)
				if mathish {
					hasUnicodeSymbol = true
				}
				if unicode.IsSpace(r) && !mathish {
					symbolOnly = false
					break
				}
			}
			if symbolOnly && hasUnicodeSymbol && !hasPunct && psCtx.psLastText.active {
				if len(runes) == 1 {
					r0 := runes[0]
					switch r0 {
					case '(', ')', '[', ']', '{', '}', '<', '>', '|':
			...
			if symbolOnly && hasUnicodeSymbol && !hasPunct && psCtx.psLastText.active {
				...
				if math.Abs(renderMatrix.Y0-psCtx.psLastText.yUser) <= tolY {
					...
					if symAdv > 0 && symAdv <= fontSize*1.6 {
						...
						} else if gap < desired*0.1 {
							prevRunes := []rune(strings.TrimSpace(psCtx.psLastText.text))
							prevShortWord := len(prevRunes) > 0 && len(prevRunes) <= 2
							if prevShortWord {
								for _, pr := range prevRunes {
									if !(unicode.IsLetter(pr) || unicode.IsNumber(pr)) {
										prevShortWord = false
										break
									}
								}
							}
							if prevShortWord && len(runes) == 1 {
								r0 := runes[0]
								if r0 >= 0x2190 && r0 <= 0x21FF {
									shift := math.Min(spaceAdv*0.7, fontSize*0.35)
									renderMatrix.X0 -= shift
								}
							}
						}
					}
	...
	if enablePSLabelGapClamp && psPreferText && psCtx != nil && candidateDecoded != "" && psCtx.psLastText.active {
		...
		prevRunesGeneric := []rune(prevTrim)
		curRunesGeneric := []rune(curTrim)
		if len(prevRunesGeneric) >= 4 && len(curRunesGeneric) == 1 {
			cr := curRunesGeneric[0]
			prevAllASCII := true
			for _, rr := range prevRunesGeneric {
				if rr > 0x7F || !unicode.IsLetter(rr) {
					prevAllASCII = false
					break
				}
			}
			if prevAllASCII && cr > 0x7F && unicode.IsLetter(cr) {
				tolY := math.Max(2.0, math.Max(psCtx.psLastText.fontSize, fontSize)*0.5)
				if math.Abs(renderMatrix.Y0-psCtx.psLastText.yUser) <= tolY {
					prevEnd := psCtx.psLastText.xEndUser
					if psCtx.psLastText.xInkEndUser > 0 {
					...
					approxW := estimateTextWidthSimple(prevTrim, psCtx.psLastText.fontSize)
					if approxW > 0 {
					...
					maxGap := space * 1.35
					if gap < minGap {
						renderMatrix.X0 = prevEnd + minGap
					} else if gap > maxGap {
						renderMatrix.X0 = prevEnd + maxGap
					}
				}
			}
		}

		if len(prevRunesGeneric) == 1 && len(curRunesGeneric) > 0 {
			pr := prevRunesGeneric[0]
			r0 := curRunesGeneric[0]
			prevIsWord := unicode.IsLetter(pr) || unicode.IsNumber(pr)
			curStartsWord := unicode.IsLetter(r0) || unicode.IsNumber(r0)
			if prevIsWord && curStartsWord {
				tolY := math.Max(2.0, math.Max(psCtx.psLastText.fontSize, fontSize)*0.5)
				isScript := psCtx.psLastText.fontSize <= fontSize*0.85 && math.Abs(renderMatrix.Y0-psCtx.psLastText.yUser) <= tolY
				if isScript {
					prevEnd := psCtx.psLastText.xEndUser
					if psCtx.psLastText.xInkEndUser > 0 {
						prevEnd = psCtx.psLastText.xInkEndUser
					...
					gap := renderMatrix.X0 - prevEnd
					maxGap := psCtx.psLastText.spaceAdvUser * 0.75
					if maxGap <= 0 {
						maxGap = fontSize * 0.30
					}
					if maxGap > fontSize*0.35 {
						maxGap = fontSize * 0.35
					}
					if gap > maxGap {
						renderMatrix.X0 = prevEnd + maxGap
					}
		...
		if textState.Font != nil && isTeXMathFont(textState.Font.BaseFont) {
			...
			if len(curRunes) == 1 {
				...
				if !isScript {
					prevEndsWord := len(prevRunes) >= 4 && unicode.IsLetter(prevRunes[len(prevRunes)-1])
					prevAllASCII := true
					for _, rr := range prevRunes {
						if rr > 0x7F || !unicode.IsLetter(rr) {
							prevAllASCII = false
							break
						}
					}
					if prevEndsWord && prevAllASCII && (unicode.IsLetter(cr) || unicode.IsNumber(cr)) {
						prevEnd := psCtx.psLastText.xEndUser
						if psCtx.psLastText.xInkEndUser > 0 {
							prevEnd = psCtx.psLastText.xInkEndUser
						}
					...
					} else if prevEndsWord && prevAllASCII && cr > 0x7F && unicode.IsLetter(cr) {
						tolY := math.Max(2.0, math.Max(psCtx.psLastText.fontSize, fontSize)*0.5)
						if math.Abs(renderMatrix.Y0-psCtx.psLastText.yUser) <= tolY {
							prevEnd := psCtx.psLastText.xEndUser
							if psCtx.psLastText.xInkEndUser > 0 {
				...
				if len(prevRunes) == 1 {
					...
					if prevMathish && curWord && isScript {
						...
						gap := renderMatrix.X0 - prevEnd
						maxGap := math.Max(fontSize, psCtx.psLastText.fontSize) * 0.25
						if maxGap < fontSize*0.06 {
							maxGap = fontSize * 0.06
						} else if maxGap > fontSize*0.60 {
							maxGap = fontSize * 0.60
						}
						if gap > maxGap {
							renderMatrix.X0 = prevEnd + maxGap
						}
			...
			if !(textState.Font != nil && isTeXMathFont(textState.Font.BaseFont)) {
				...
				prevRunes := []rune(prev)
				curRunes := []rune(strings.TrimSpace(candidateDecoded))
				if len(prevRunes) >= 4 && len(curRunes) == 1 {
					cr := curRunes[0]
					prevAllASCII := true
					for _, rr := range prevRunes {
						if rr > 0x7F || !unicode.IsLetter(rr) {
					...
					if prevAllASCII && cr > 0x7F && unicode.IsLetter(cr) {
						...
						if math.Abs(renderMatrix.Y0-psCtx.psLastText.yUser) <= tolY {
							...
							approxW := estimateTextWidthSimple(prev, psCtx.psLastText.fontSize)
							if approxW > 0 {
								approxEnd := psCtx.psLastText.xStartUser + approxW
								if approxEnd > 0 && approxEnd < prevEnd {
									prevEnd = approxEnd
							...
							spaceAdv := fontSize * 0.30
							if textState.Font != nil {
								if adv := textState.GlyphAdvance(32, true); adv > 0 {
									spaceAdv = adv
								}
							}
							desired := spaceAdv
							if desired < fontSize*0.18 {
								desired = fontSize * 0.18
							} else if desired > fontSize*0.70 {
								desired = fontSize * 0.70
							}
							if gap > desired*2.0 {
								renderMatrix.X0 = prevEnd + desired
							}
	...
	if psPreferText && psCtx != nil && psCtx.psLastText.active {
		...
		if len(prevRunes) > 0 && len(curRunes) > 0 {
			...
			if len(curRunes) == 1 {
				r0 := curRunes[0]
				curIsShort = (unicode.IsLetter(r0) || unicode.IsNumber(r0) || unicode.IsSymbol(r0) || unicode.IsPunct(r0)) && !unicode.IsSpace(r0)
			}
			prevFamily := strings.TrimSpace(psCtx.psLastText.fontFamily)
			currentIsMath := strings.EqualFold(strings.TrimSpace(fontFamily), "math")
			prevIsMath := strings.EqualFold(prevFamily, "math")

			prevHasTrailingSpace := false
			...
			if prevEndsWord && curIsShort && currentIsMath && !prevIsMath && prevFamily != "" && !prevHasTrailingSpace {
				...
				if math.Abs(renderMatrix.Y0-psCtx.psLastText.yUser) <= tolY {
					...
					if prevEnd <= 0 {
						approxW := estimateTextWidthSimple(prevTrim, psCtx.psLastText.fontSize)
						sxx := psCtx.psLastText.mXX
						if sxx == 0 {
							sxx = 1
						}
						prevEnd = psCtx.psLastText.xStartUser + approxW*sxx
					}
					space := psCtx.psLastText.spaceAdvUser
					if space <= 0 {
						sxx := psCtx.psLastText.mXX
					...
					if gap >= space*0.20 {
						...
						spaceMatrix := &Matrix{XX: smXX, XY: smXY, YX: smYX, YY: smYY, X0: spaceX0, Y0: psCtx.psLastText.yUser}
						ctx.GopdfCtx.Save()
	...
	computeRunScaleAndAdvance := func(runText string, cids []uint16) (scaleX float64, adv float64) {
		...
		pdfAdvOK := fontWidthMetricsAvailable(textState.Font)
		if psPreferText && asciiOnly && textState.CharSpacing == 0 && textState.WordSpacing == 0 && !pdfAdvOK {
			est := estimateTextWidthSimple(runText, fontSize)
			if est <= 0 {
				est = drawAdv
			}
			return hScale, est * hScale
		}
	...
	delimiterAdjust := func(glyphName string) (dyFactor float64, scaleY float64, ok bool) {
		...
		switch glyphName {
		case "parenleftbig", "parenrightbig", "bracketleftbig", "bracketrightbig", "braceleftbig", "bracerightbig":
			return -0.95, 1.70, true
		case "parenleftBig", "parenrightBig", "bracketleftBig", "bracketrightBig", "braceleftBig", "bracerightBig":
			return -1.10, 2.00, true
		case "parenleftBigg", "parenrightBigg", "bracketleftBigg", "bracketrightBigg", "braceleftBigg", "bracerightBigg":
			return -1.25, 2.35, true
		case "parenleftBiggg", "parenrightBiggg", "bracketleftBiggg", "bracketrightBiggg", "braceleftBiggg", "bracerightBiggg":
	...
	var isTeXMathBaseFont func(baseFont string) bool
	var isTeXLargeOperatorRune func(r rune) bool
	var isASCIIDelimiterRune func(r rune) bool
	var renderTeXMathFallbackGlyphs func(runText string, cids []uint16) (adv float64, ok bool)
	...
	renderRun := func(runText string, cids []uint16, scaleX float64) (adv float64, override bool) {
		...
		renderRunAtX := func(x float64, t string, c []uint16, sx float64) {
			...
			if psPreferText && isMathFamily {
				...
				if len(tr) == 1 {
					...
					if dy == 0 {
						r := tr[0]
						if (unicode.IsSymbol(r) || unicode.IsPunct(r)) && !isASCIIDelimiterRune(r) {
							dy = autoSymbolDy(t)
						}
					}
				}
			...
			if scaledFont != nil {
				...
				right := (x + dx) + (ext.XBearing+ext.Width)*sx
				if right > inkEndX {
					inkEndX = right
				}
			}

			if sx != 1.0 || dx != 0.0 || dy != 0.0 || sy != 1.0 {
				ctx.GopdfCtx.Save()
				...
				if dx != 0.0 || dy != 0.0 {
					...
					ctx.GopdfCtx.Translate(dx/invSX, dy/invSY)
				}
				ctx.GopdfCtx.MoveTo(0, 0)
				layout.SetText(t)
		...
		allowPerGlyph := psPreferText && perGlyphUseful

		if allowPerGlyph {
			runes := []rune(runText)
			if len(runes) == len(cids) && len(runes) <= 64 {
				localX := currentX
				localAdv := 0.0
				for i, r := range runes {
					cid := cids[i]
					...
					isTeXMath := psPreferText && textState.Font != nil && isTeXMathBaseFont(textState.Font.BaseFont)
					glyphWidth := 0.0
					if textState.Font != nil {
						glyphWidth = textState.Font.GetWidth(cid)
					}
					isCJK := isCJKCharacterRune(r)
					if glyphWidth == 0 {
						if isCJK {
							glyphWidth = 1000.0
						} else if textState.Font != nil && textState.Font.DefaultWidth > 0 {
							glyphWidth = textState.Font.DefaultWidth
						} else if textState.Font != nil && textState.Font.MissingWidth > 0 {
							glyphWidth = textState.Font.MissingWidth
						} else {
							glyphWidth = 500.0
						}
					}
					pdfGlyphAdv := glyphWidth * fontSize / 1000.0 * hScale
					extra := 0.0
					if isCJK {
						if textState.CharSpacing != 0 {
							extra += textState.CharSpacing * 0.5
						}
					} else {
						extra += textState.CharSpacing
					}
					if isSpace {
						extra += textState.WordSpacing
					}
					extra *= hScale
					...
					if isTeXMath {
						pdfGlyphAdv = drawAdv * hScale
					}
					advStep := pdfGlyphAdv + extra
					sx := 1.0
					if !isTeXMath && pdfGlyphAdv > 0 && drawAdv > 0 {
						unclamped := pdfGlyphAdv / drawAdv
						sx = unclamped
						minRatio, maxRatio := 0.25, 2.5
						if sx < minRatio {
							sx = minRatio
					...
					if isTeXMath {
						name := ""
						if len(textState.Font.CodeToGlyphName) > 0 {
							if n, ok := textState.Font.CodeToGlyphName[byte(cid&0xFF)]; ok {
								name = strings.ToLower(strings.TrimPrefix(n, "/"))
							}
						}
						isDelimiter := strings.Contains(name, "paren") || strings.Contains(name, "bracket") || strings.Contains(name, "brace")
						isLargeOp := isTeXLargeOperatorRune(r) || strings.Contains(name, "summation") || strings.Contains(name, "product") || strings.Contains(name, "integral") || strings.Contains(name, "coproduct")
						if isDelimiter || isLargeOp || (r > 0x7F && (unicode.IsSymbol(r) || unicode.IsPunct(r)) && !psMathRunesRepresentable(string(r))) {
							saved := currentX
							currentX = localX
							if advOutline, ok := renderTeXMathFallbackGlyphs(string(r), []uint16{cid}); ok {
								currentX = saved
								localX += advOutline
								localAdv += advOutline
								continue
	...
	isTeXLargeOperatorRune = func(r rune) bool {
		switch r {
		case 0x220F, 0x2210, 0x2211, 0x222B, 0x222C, 0x222D, 0x222E, 0x222F, 0x2230, 0x22C2, 0x22C3:
			return true
		default:
			return false
		}
	...
	isASCIIDelimiterRune = func(r rune) bool {
		switch r {
		case '(', ')', '[', ']', '{', '}', '<', '>', '|':
			return true
		default:
	...
	if len(array) > 0 {
		...
		if psPreferText {
			isMathLike := strings.EqualFold(fontFamily, "math")
			if textState.Font != nil && isTeXMathFont(textState.Font.BaseFont) {
				isMathLike = true
			}
			psBodyText = !isMathLike
			enablePSTextSpaceSynthesis = psBodyText
		}

		mergeTol := fontSize * 0.06
		if psPreferText && psBodyText {
			mergeTol = fontSize * 0.18
		}
		pendingKerning := 0.0
		pendingSpaces := 0
		var bufText string
		...
		flushBuf := func() {
			...
			outline := psPreferText && (psForceOutline || decodedText == "")
...
updateMatrix:
	...
	if psPreferText && psCtx != nil {
		...
		psCtx.psLastText.xStartUser = renderMatrix.X0
		psCtx.psLastText.fontSize = fontSize
		psCtx.psLastText.fontFamily = fontFamily
		psCtx.psLastText.mXX = renderMatrix.XX
		psCtx.psLastText.mXY = renderMatrix.XY
		psCtx.psLastText.mYX = renderMatrix.YX
		psCtx.psLastText.mYY = renderMatrix.YY
		hs := textState.HorizontalScaling / 100.0
		if hs <= 0 {
			hs = 1
		}
		sp := scaledFont.TextExtents(" ").XAdvance
		if sp <= 0 {
			sp = fontSize * 0.33
		}
		psCtx.psLastText.spaceAdvUser = sp * hs * renderMatrix.XX
		psCtx.psLastText.xEndUser = renderMatrix.X0 + currentX*renderMatrix.XX
		psCtx.psLastText.xInkEndUser = 0
		if inkEndX > 0 {
			psCtx.psLastText.xInkEndUser = renderMatrix.X0 + inkEndX*renderMatrix.XX
		}
		if lastRenderedText != "" {
...
func shouldOutlineTextInPS(font *Font, decodedText string, cids []uint16, stats TextDecodeStats) bool {
	...
	if (font.Subtype == "Type0" || font.Subtype == "/Type0") && stats.CIDCount > 0 {
		if stats.ToUnicodeHit == 0 || stats.Replaced > 0 {
			return true
		}
	}
	...
	if isTeXMathBaseFontName(font.BaseFont) {
		if decodedText != "" && psMathRunesRepresentable(decodedText) {
			hasNonASCII := false
...
func psMathRunesRepresentable(s string) bool {
	m := psDefaultMathRuneToByte()
	for _, r := range s {
		if r == 0 {
			return false
		}
...
func fontWidthMetricsAvailable(font *Font) bool {
	if font == nil {
		return false
	}
	hasWidths := font.Widths != nil &&
		(len(font.Widths.Widths) > 0 || len(font.Widths.CIDWidths) > 0 || len(font.Widths.CIDRanges) > 0)
	if hasWidths {
		return true
	}
	if font.DefaultWidth > 0 && math.Abs(font.DefaultWidth-1000.0) > 1e-3 {
		return true
	}
	if font.embeddedWidthAvailable() {
		return true
	}
	return false
}
