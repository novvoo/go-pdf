...
func NewGradientRenderer(ctx Context) *GradientRenderer {
...
func (gr *GradientRenderer) RenderLinearGradient(shading *Shading) (Pattern, error) {
	...
	// 创建 Gopdf 线性渐变
	pattern := NewPatternLinear(x0, y0, x1, y1)
...
func (gr *GradientRenderer) RenderRadialGradient(shading *Shading) (Pattern, error) {
	...
	// 创建 Gopdf 径向渐变
	pattern := NewPatternRadial(x0, y0, r0, x1, y1, r1)
...
func (gr *GradientRenderer) addColorStops(pattern Pattern, shading *Shading) error {
	if shading.Function == nil {
		// 没有函数，使用默认黑到白渐变
		if gradPattern, ok := pattern.(GradientPattern); ok {
